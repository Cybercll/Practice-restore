...
void printone(int a,student(&p)[5]);//输出单一学生信息
...
int main()//main 函数
{
	...
	while (stop) {
		...
		case 4: {max(stusort); continue; }//考的最好的
...
{
	cout << "**************************" << endl << "请输入1~6内数字" << endl << "1. 输出学生信息" << endl << "2. 计算总平均成绩" << endl << "3. 查询指定学号学生信息" << endl << "4. 考得最好的学生信息" << endl << "5. 按学生的最终成绩降序排序" << endl << "6. 结束" << endl <<"0. 清屏"<<endl<< "**************************" << endl;
	cout << "请输入选项" << endl;
}
void sort(student(&t)[5])
{
	student temp;
	for (int i = 0; i < 5; i++)//双层防冗余
	{
		for (int p = i + 1; p < 5; p++)
		{
			if (t[i].sc < t[p].sc)
